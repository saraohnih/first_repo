...
class Module(object):
    ...
    def get_important_dates_overview(self):
        print("Important dates for %s:" %self.title)
        for date_tpl in self.dates:
            print("%s on %s" %(date_tpl[0], date_tpl[1]))
    ...
    def add_module_element(self, class_name, date):
        ...
        element.add_important_date(date)
...
class Thesis(Module):
    ...
    def __str__(self):
        return "Bachelor Thesis on the topic: %s in the Research Group %s" \
               % (self.topic, self.research_group)
